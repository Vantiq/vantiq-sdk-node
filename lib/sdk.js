...
// 
Vantiq.prototype.subscribe = function(resource, name, operation, callback) {
    var path;
    switch(resource) {
    case 'sources':
    case 'topics':
    ...
    return this.session.subscribe(path, {}, callback);
};

Vantiq.prototype.subscribePersistent = function(resource, name, parameters, callback) {
    var path;
    switch(resource) {
        case 'sources':
            path = '/' + resource + '/' + name;
        ...
        case 'types':
            if(arguments.length != 4) {
                return Promise.reject(new Error('Cannot make a persistent subscription to type events'));
            }
            break;
        default:
            return Promise.reject(new Error('Only "topics" and "sources" support persistent subscribe'));
    }
